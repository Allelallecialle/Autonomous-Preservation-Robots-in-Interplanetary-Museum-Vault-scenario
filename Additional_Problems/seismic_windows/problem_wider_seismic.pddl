...
;;
;; To run with Problem 4 domain.pddl (check README for commands to run)
;; 
;; Checks if a single wide window in the middle of the plan t[8, 28] is harder to plan.
;; Do the curators route around it like before, or does it produce idle waiting?
;; ==========================================================================

(define (problem imv-transport-wide-seismic-p4)
  (:domain imv-transport-temporal)
  ...
  (:init
    ...
    ;; seismic windows: one wide seismic window replaces the two short ones in the middle of the plan
    (seismic-clear)
    (at 8 (seismic-active))
    (at 8 (not (seismic-clear)))
    (at 28 (not (seismic-active)))
    (at 28 (seismic-clear))
  )
